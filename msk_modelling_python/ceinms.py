'''
This module was created to allow running CEINMS from Python, using custom function

CEINMS implementations from:

Hambly, M. J., Worsey, M. T. O., Lloyd, D. G., & Pizzolato, C. -2025-. Improving Calibration of EMG-Informed Neuromusculoskeletal Models Through Differentiable Physics and Muscle Synergies. IEEE Transactions on Biomedical Engineering.

Usage:

    python -m msk_modelling_python\ceinms.py

Note: Template settings are included but these should be updated based on the specific use case


Author: Basílio Gonçalves, 2026, https://github.com/basgoncalves
...
EMG_MAPPING = 'Powerlifting_s1' # 'Powerlifting_s1', 'Powerlifting_s2', 'running_fai'
...
def unzip_torch_cpu():
    """
    Unzip the CEINMS executable from a zip file.
    
    Args:
        zip_file_path (str): Path to the zip file containing the CEINMS executable.
        extract_to (str): Directory where the executable should be extracted.
    """

    torch_path = os.path.join(utils.CODE_DIR, 'executables', 'torch_cpu.zip')
    if not os.path.exists(torch_path):
        print(f"Zip file not found: {torch_path}")
        return
    
    extract_to = os.path.join(utils.CODE_DIR, 'executables')   
    shutil.unpack_archive(torch_path, extract_to)
    print(f"CEINMS executable extracted to: {extract_to}")
